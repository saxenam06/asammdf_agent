...
class AdaptiveExecutor:
    ...
    def get_all_unresolved_failures_for_task(self, task: str) -> List[Dict[str, Any]]:
        """
        Get all unresolved failures across all KB items for this task

        Args:
            task: Current task being executed

        Returns:
            List of dicts with {kb_id, learning_index, learning, failed_step_num}
        """
        ...
        try:
            ...
            # Search all KB items for unresolved failures matching this task
            for item in catalog_data:
                kb_id = item.get("knowledge_id")
                kb_learnings = item.get("kb_learnings", [])

                for idx, learning in enumerate(kb_learnings):
                    # Check if it's a failure learning (has original_error field)
                    if "original_error" in learning:
                        # Check if it's unresolved (empty recovery_approach)
                        if not learning.get("recovery_approach", "").strip():
                            # Check if it matches current task
                            learning_task = learning.get("task", "")
                            if learning_task == task or learning_task in task or task in learning_task:
                                unresolved_failures.append({
                                    "kb_id": kb_id,
                                    "learning_index": idx,
                                    "learning": learning,
                                    "failed_step_num": learning.get("step_num", -1)
                                })

            return unresolved_failures

        except Exception as e:
            print(f"  [Warning] Could not load unresolved failures: {e}")
            return []
    ...
    def analyze_recovery(
        ...
        successful_plan_actions: List[ActionSchema]
    ) -> str:
        """
        ...
        if failed_step_num < len(successful_plan_actions):
            ...
            # Compare KB sources
            if failed_kb_source != current_kb_source:
                if failed_kb_source and current_kb_source:
                    recovery_parts.append(
                        f"Used different KB item: '{failed_kb_source}' → '{current_kb_source}'"
                    )
                elif current_kb_source and not failed_kb_source:
                    recovery_parts.append(
                        f"Applied KB guidance from '{current_kb_source}'"
                    )
                elif not current_kb_source and failed_kb_source:
                    recovery_parts.append(
                        f"Abandoned KB item '{failed_kb_source}' approach, used alternative"
                    )

        # Check for additional State-Tool calls in successful plan
        state_tools_count = sum(
            1 for action in successful_plan_actions
            if action.tool_name == "State-Tool"
        )
        if state_tools_count > 0:
            recovery_parts.append(
                f"Plan included {state_tools_count} State-Tool check(s) for better UI awareness"
            )
    ...
    def update_recovery_in_kb(
        ...
        """
        Update the recovery_approach field in an existing failure learning

        Args:
            kb_id: Knowledge base item ID
            learning_index: Index of the learning to update
            recovery_approach: Description of successful recovery approach
        """
        catalog_path = "agent/knowledge_base/parsed_knowledge/knowledge_catalog.json"
        ...
        try:
            ...
            # Find KB item and update learning
            kb_found = False
            for item in catalog_data:
                if item.get("knowledge_id") == kb_id:
                    kb_learnings = item.get("kb_learnings", [])

                    if 0 <= learning_index < len(kb_learnings):
                        kb_learnings[learning_index]["recovery_approach"] = recovery_approach

                        # Increase trust score slightly (recovery success)
                        current_trust = item.get("trust_score", 1.0)
                        item["trust_score"] = min(1.0, current_trust * 1.05)

                        kb_found = True
                        print(f"  [Recovery] Updated '{kb_id}': Added recovery approach, trust={item['trust_score']:.2f}")
                    else:
                        print(f"  [Warning] Learning index {learning_index} out of range")
                        return
            ...
            print(f"  [Recovery] Catalog updated with recovery approach")

        except Exception as e:
            print(f"  [Error] Failed to update recovery approach: {e}")
            import traceback
            traceback.print_exc()
